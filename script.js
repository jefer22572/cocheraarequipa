...
function calcular(entradaMs, tipo) {
    ...
    const HORA_INICIO_NOCHE = 23; // 11 PM
    const HORA_FIN_NOCHE = 8;    // 8 AM
    ...
    if (res.nocheMonto > 0) {
        htmlDetalle += `
            <div class="flex justify-between text-orange-600 font-bold text-xs">
                <span>Tarifa Noche (11PM-8AM):</span> <span>S/ ${res.nocheMonto.toFixed(2)}</span>
            </div>
        `;
    }
...
function imprimirTicket(t, modo) {
    ...
    area.innerHTML = `
        <div style="text-align:center; font-weight:bold;">
            <span style="font-size:14pt;">COCHERA AREQUIPA</span><br>
...
function exportarExcel() {
    ...
    XLSX.writeFile(wb, "Reporte_Cochera_Arequipa.xlsx");
}
